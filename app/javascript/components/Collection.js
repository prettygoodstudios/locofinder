  ...
  const photoGrid = photoArray.map((photo,i) =>
      <LazyLoad offset={100} placeholder={placeholderDiv}>
        <div key={photo.id} className={parseDigitToString(props.colls)+" columns img-card img-card-"+(12/props.colls)}>
          ...
          <br/>
          <p>
            <UserTag profileImg={props.users[i].profile_img} width={props.users[i].width} height={props.users[i].height} zoom={props.users[i].zoom} offsetX={props.users[i].offsetX} offsetY={props.users[i].offsetY} rootUrl={props.rootUrl} id={props.users[i].id} display={props.users[i].display} /> { photo.caption ? `"${photo.caption}"` : ""} <a href={props.rootUrl+"location/"+props.locations[i].slug} className="img-card-link" data-turbolinks="false"><img src="https://s3-us-west-2.amazonaws.com/staticgeofocus/70+by+70.png" style={{width: 20, height: 20, display: 'inline'}} />{props.locations[i].title}</a> { props.currentUser != null && props.currentUser.verified && <a href={props.rootUrl+"report/new?photo"+photo.id} className="img-card-link">Report Content</a> }
          </p>
